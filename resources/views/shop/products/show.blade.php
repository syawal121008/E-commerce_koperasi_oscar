            ...
            <div class="p-4 sm:p-6">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4 lg:gap-6">
                    @foreach($relatedProducts as $related)
                        <div class="related-product-card bg-gradient-to-r from-blue-50 to-blue-100 border border-blue-300 rounded-lg overflow-hidden shadow hover:shadow-lg transition-all duration-300 hover:-translate-y-1 flex flex-col">
                            
                            <!-- Bungkus gambar + info produk jadi link -->
                            <a href="{{ route('shop.products.show', $related->product_id) }}" class="flex-1 flex flex-col">
                                <div class="aspect-square overflow-hidden">
                                    <img src="{{ $related->image ? asset('storage/' . $related->image) : 'https://via.placeholder.com/200x200?text=No+Image' }}" 
                                         alt="{{ $related->name }}" 
                                         class="w-full h-full object-cover hover:scale-105 transition-transform duration-300">
                                </div>
                                <div class="related-product-info p-2 sm:p-3 lg:p-4 flex flex-col flex-1">
                                    ...
                                    <div class="flex items-center mb-2 sm:mb-3 min-h-[1rem] sm:min-h-[1.25rem]">
                                        @php $rating = $related->avg_rating ?? 4; @endphp
                            ...
                            <!-- Action: tombol keranjang doang -->
                            <div class="related-product-actions p-2 sm:p-3 lg:p-4">
                                <button onclick="addToCartFromRelated('{{ $related->product_id }}', '{{ $related->admin_id }}')" 
                                        class="w-full bg-blue-600 hover:bg-blue-700 text-white py-1.5 sm:py-2 px-2 sm:px-3 rounded text-xs sm:text-sm transition-colors font-medium flex items-center justify-center">
