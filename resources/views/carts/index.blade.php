...
<x-app-layout>
    ...
    <div class="container mx-auto px-2 sm:px-4 md:px-6 py-6">
        <div class="max-w-7xl mx-auto">
            ...
            <div id="empty-cart" class="text-center bg-white rounded-lg shadow-sm p-8 md:p-16 hidden">
                <div class="mb-8">
                    <i class="fas fa-shopping-cart text-5xl md:text-6xl text-gray-300"></i>
                </div>
                <h2 class="text-xl md:text-2xl font-semibold text-gray-800 mb-4">Keranjang Belanja Kosong</h2>
                <p class="text-gray-600 mb-8">Belum ada produk yang ditambahkan ke keranjang Anda.</p>
                <a href="{{ route('shop.index') }}" 
                   class="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 md:py-3 md:px-6 rounded-lg inline-flex items-center transition-colors text-base">
                        ...
                        <div class="flex items-center justify-between py-2 md:py-3">
                            <div class="flex items-center space-x-2 sm:space-x-4">
                                <label class="flex items-center cursor-pointer">
                                    ...
                                    <span class="ml-2 text-gray-700 text-sm sm:text-base">Pilih Semua</span>
                                </label>
                                <span class="text-gray-300 hidden sm:inline">|</span>
                                <button type="button" id="delete-selected-bottom" class="text-red-500 hover:text-red-700 disabled:opacity-50 disabled:cursor-not-allowed">
                                    <span class="hidden sm:inline">Hapus</span> (<span id="selected-count-bottom">0</span>)
                                </button>
                            </div>

                            <div class="flex items-center space-x-2 sm:space-x-4 md:space-x-6">
                                <div class="text-right">
                                    <div class="hidden sm:block text-sm text-gray-500">Total (<span id="selected-items-count">0</span> item):</div>
                                    <div class="text-base sm:text-xl font-bold text-blue-500" id="total-price">Rp 0</div>
                                </div>
                                <button type="button" 
                                        id="checkout-btn" 
                                        class="bg-blue-500 hover:bg-blue-600 text-white font-semibold px-4 py-2 sm:px-6 sm:py-3 rounded-lg disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors text-sm sm:text-base"
                                        disabled>
                                    Checkout
                                </button>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" name="cash_amount" id="cashAmount" 
                                           class="w-full border border-gray-300 rounded-lg pl-8 pr-3 py-2 bg-gray-50" readonly>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <style>
        ...
        .quantity-controls button {
            border: none;
            background: white;
            /* padding: 0.5rem 0.75rem; */ /* Adjusted in JS */
            color: #6b7280;
            transition: all 0.2s;
        }
    ...
    <script>
        ...
        class ShopeeCart {
                ...
                Object.entries(groupedItems).forEach(([adminId, items]) => {
                    ...
                    html += `
                        <div class="bg-white rounded-lg shadow-sm border overflow-hidden">
                            <div class="shop-header p-3 sm:p-4">
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="text-blue-200 text-sm flex items-center">
                                        <i class="fas fa-shopping-bag mr-1"></i>
                                        <span class="hidden sm:inline">${items.length} produk</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
                                <div class="flex items-center space-x-2 sm:space-x-4">
                                    <label class="flex items-center cursor-pointer">
                                        ...
                                        <span class="ml-2 sm:ml-3 text-gray-700 font-medium">Pilih Semua</span>
                                    </label>
                                    <span class="text-gray-300 hidden sm:inline">|</span>
            ...
            renderCartItem(item, isLast = false) {
                ...
                return `
                    <div class="cart-item product-card p-3 sm:p-4 ${item.selected ? 'selected' : ''} ${isLast ? '' : 'border-b border-gray-100'}" 
                        ...
                        <div class="flex items-start space-x-3 sm:space-x-4">
                            ...
                            <div class="flex-shrink-0">
                                <img src="${imageUrl}" 
                                     alt="${item.name}" 
                                     class="w-20 h-20 sm:w-24 sm:h-24 object-cover rounded-lg border-2 border-gray-200"
                            ...
                            <div class="flex-grow min-w-0">
                                <div class="flex justify-between items-start">
                                    <div class="flex-grow pr-4">
                                        <h3 class="text-gray-800 font-medium text-sm sm:text-base line-clamp-2 mb-2">${item.name}</h3>
                                        <div class="flex items-center space-x-2 mb-3">
                                            <span class="price-text text-base sm:text-lg font-bold">
                                    ...
                                    <button type="button" 
                                            class="text-gray-400 hover:text-red-500 p-1 sm:p-2 delete-item-btn rounded-full hover:bg-red-50 transition-colors"
                                            data-item-id="${item.cart_id}"
                                            title="Hapus item">
                                ...
                                <div class="flex flex-col items-end gap-y-2 sm:flex-row sm:items-center sm:justify-between mt-4">
                                    <div class="quantity-controls">
                                        <button type="button" 
                                                class="quantity-btn px-2 py-1 sm:px-3 sm:py-2 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed" 
                                        ...
                                        <input type="text" 
                                               readonly 
                                               value="${item.quantity}" 
                                               class="quantity-display w-12 sm:w-16 text-center border-0 bg-white focus:ring-0 text-gray-800 font-medium py-1 sm:py-2"
                                               data-item-id="${item.cart_id}">
                                        <button type="button" 
                                                class="quantity-btn px-2 py-1 sm:px-3 sm:py-2 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed" 
                                    ...
                                    <div class="text-right">
                                        <div class="price-text font-bold text-base sm:text-lg item-subtotal" data-item-id="${item.cart_id}">
            ...
            addEventListeners() {
                ...
                document.querySelectorAll('.shop-checkbox').forEach(checkbox => {
                    checkbox.addEventListener('change', (e) => {
                        ...
                        const shopItems = this.items.filter(item => item.admin_id == adminId); // Use == for type coercion with data attributes
            ...
            updateCheckboxes() {
                ...
                document.querySelectorAll('.shop-header').forEach(header => {
                    const shopCheckbox = header.querySelector('.shop-checkbox');
                    if (!shopCheckbox) return;

                    const adminId = shopCheckbox.dataset.adminId;
        ...
        function updateMixedPayment() {
            ...
            let balanceUsed = parseInt(balanceUsedInput.value) || 0;
            const userBalance = {{ auth()->user()->balance ?? 0 }};

            // Prevent using more balance than available
            if (balanceUsed > userBalance) {
                balanceUsed = userBalance;
                balanceUsedInput.value = userBalance;
            }
