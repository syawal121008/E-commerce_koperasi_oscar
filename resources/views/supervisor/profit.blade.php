                    ...
                    <div class="p-4 md:p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3 md:ml-4 min-w-0 flex-1">
                                ...
                                <p class="text-lg md:text-2xl font-bold text-gray-900 truncate" title="Rp {{ number_format($summary['total_revenue'], 0, ',', '.') }}">
                    ...
                    <div class="p-4 md:p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3 md:ml-4 min-w-0 flex-1">
                                ...
                                <p class="text-lg md:text-2xl font-bold text-gray-900 truncate" title="Rp {{ number_format($summary['total_modal_cost'], 0, ',', '.') }}">
                                    Rp {{ number_format($summary['total_modal_cost'], 0, ',', '.') }}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-4 md:p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3 md:ml-4 min-w-0 flex-1">
                                ...
                                <p class="text-lg md:text-2xl font-bold {{ $summary['total_profit'] >= 0 ? 'text-green-600' : 'text-red-600' }} truncate" title="Rp {{ number_format($summary['total_profit'], 0, ',', '.') }}">
                    ...
                    <div class="p-4 md:p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3 md:ml-4 min-w-0 flex-1">
                                ...
                                <p class="text-lg md:text-2xl font-bold {{ $summary['profit_margin'] >= 0 ? 'text-green-600' : 'text-red-600' }} truncate" title="{{ number_format($summary['profit_margin'], 1) }}%">
...
<script>
// --- Event Listener ---
// Panggil fungsi ini saat DOM ready
document.addEventListener('DOMContentLoaded', function() {
    // Initialize period change handler on page load
    handlePeriodChange();
    
    // Initialize charts - TAMBAHKAN DELAY untuk memastikan data sudah dimuat
    setTimeout(() => {
        initializeCharts();
    }, 100);
...
function initializeCharts() {
    ...
    if (dailySalesCtx) {
        // PERBAIKAN: Handle data dengan lebih aman
        const dailySalesData = @json($daily_sales ?? collect());
        
        // Convert data ke format yang diperlukan Chart.js
        const labels = Object.keys(dailySalesData);
        const revenueData = labels.map(label => dailySalesData[label]?.revenue || 0);
        const profitData = labels.map(label => dailySalesData[label]?.profit || 0);
        ...
        if (labels.length === 0) {
            dailySalesCtx.getContext('2d').fillText('Tidak ada data untuk ditampilkan', 50, 50);
            return;
        }
    ...
    if (profitCtx) {
        const totalModal = {{ $summary['total_modal_cost'] ?? 0 }};
        ...
        if (totalModal === 0 && totalProfit === 0) {
            profitCtx.getContext('2d').fillText('Tidak ada data untuk ditampilkan', 50, 50);
            return;
        }
