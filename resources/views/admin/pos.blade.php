<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-2">
                <!-- PERBAIKAN: Tambah tombol refresh produk -->
                <button id="refresh-products-btn" class="flex items-center px-4 py-2 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white rounded-xl transition-all transform hover:scale-105 shadow-lg">
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
                        <div class="p-8 border-b border-gray-100 dark:border-gray-700 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-700">
                            <div class="flex items-center justify-between mb-6">
                                ...
                                <!-- PERBAIKAN: Indicator status produk -->
                                <div class="text-sm text-gray-600 dark:text-gray-400">
                                    Total produk: <span id="total-products-count">{{ count($products) }}</span>
                                </div>
                            </div>
    ...
    @push('scripts')
        ...
        <script>
            ...
            // PERBAIKAN: Function untuk inisialisasi produk
            ...
            function initializeEventListeners() {
                // PERBAIKAN: Tambah event listener untuk tombol refresh
            ...
            // PERBAIKAN: Function untuk refresh produk dari server
            function refreshProducts() {
                ...
                fetch('/admin/pos/refresh-products', {
                    method: 'GET',
                    headers: {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        updateProductsGrid(data.data.products);
                        showAlert('Menu berhasil diperbarui!', 'success');
                        
                        // Update counter
                        document.getElementById('total-products-count').textContent = data.data.products.length;
                    } else {
                        showAlert('Gagal memperbarui menu', 'error');
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
                    showAlert('Terjadi kesalahan saat memperbarui menu', 'error');
                })
                .finally(() => {
                    // Restore button
            ...
            // PERBAIKAN: Function untuk update grid produk dengan data terbaru
            function updateProductsGrid(products) {
                ...
                const productCards = products.map(product => {
                    ...
                    cardElement.className = 'product-card bg-white dark:bg-gray-700 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer border border-gray-100 dark:border-gray-600 group overflow-hidden';
                    cardElement.dataset.productId = product.product_id;
                    cardElement.dataset.category = product.category_id || '';
                    cardElement.dataset.name = product.name.toLowerCase();
                    cardElement.dataset.stock = product.stock;
                    cardElement.onclick = () => addToCart(product.product_id, product.name, product.price, '{{ auth()->user()->user_id }}');
                    ...
                    cardElement.innerHTML = `
                        ...
                        <div class="p-2 text-center">
                            <h4 class="font-semibold text-xs text-gray-800 dark:text-gray-200 line-clamp-2" style="height: 2.2rem;">${product.name}</h4>
                            <p class="font-bold text-sm text-red-500 dark:text-red-400 mt-1">Rp ${product.price.toLocaleString()}</p>
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Stok: <span id="stock-${product.product_id}">${product.stock}</span></p>
                        </div>
                    `;
                    
                    // Update data structures
                    productStocks[product.product_id] = product.stock;
            ...
            function filterAndPaginateProducts() {
                ...
                filteredProducts = allProducts.filter(product => {
                    ...
                    const hasStock = stock > 0; // PERBAIKAN: Hanya tampilkan produk yang ada stoknya
            ...
            function updateStockDisplay(productId, change) {
                ...
                if (stockElement) {
                    ...
                    productStocks[productId] = currentStock;
                    
                    // PERBAIKAN: Update dataset produk juga
                    const productCard = document.querySelector(`[data-product-id="${productId}"]`);
                    if (productCard) {
                        productCard.dataset.stock = currentStock;
                        
                        // PERBAIKAN: Sembunyikan produk jika stok habis
                        if (currentStock <= 0) {
                            productCard.style.display = 'none';
                            // Refresh filter untuk update tampilan
                            filterAndPaginateProducts();
                        }
                    }
                }
            ...
            function processPayment(method) {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        // PERBAIKAN: Update stok produk di database setelah transaksi berhasil
                        if (data.data.updated_products) {
                            data.data.updated_products.forEach(productUpdate => {
                                updateRealStockDisplay(productUpdate.product_id, productUpdate.new_stock, productUpdate.is_active);
                            });
                        }
            ...
            // PERBAIKAN: Function baru untuk update stok real dari server
            function updateRealStockDisplay(productId, newStock, isActive) {
                const stockElement = document.getElementById(`stock-${productId}`);
                const productCard = document.querySelector(`[data-product-id="${productId}"]`);
                ...
                if (productCard) {
                    ...
                    // Tampilkan/sembunyikan produk berdasarkan stok dan status aktif
                    if (newStock <= 0 || !isActive) {
                        productCard.style.display = 'none';
                    } else {
            ...
            // PERBAIKAN: Auto-refresh products setiap 30 detik
            setInterval(() => {
                ...
                .then(data => {
                    if (data.success) {
                        // Update stok produk yang berubah tanpa mengganggu UI
                        data.data.products.forEach(product => {
                            const stockElement = document.getElementById(`stock-${product.product_id}`);
                            const productCard = document.querySelector(`[data-product-id="${product.product_id}"]`);
                            
                            if (stockElement && productStocks[product.product_id] !== product.stock) {
                                ...
                                if (productCard) {
                                    ...
                                    // Tampilkan produk yang baru ada stoknya
                                    if (product.stock > 0 && productCard.style.display === 'none') {
                                        productCard.style.display = 'block';
                                        filterAndPaginateProducts(); // Refresh filter
                                    }
                                }
                            }
                        });
    ...
    <style>
        ...
        * {
        ...
        /* PERBAIKAN: Style untuk produk yang disabled */
