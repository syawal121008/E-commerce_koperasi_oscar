...
class TransactionController extends Controller
{
    ...
    {
        // 1. Memulai query dengan eager loading untuk efisiensi
        //    Ini akan memuat relasi 'user' dan 'order.customer' sekali jalan.
        $query = Transaction::with(['user', 'order.customer']);

        // 2. Logika untuk hak akses berdasarkan peran (role) pengguna
        //    Jika pengguna BUKAN 'superadmin', query hanya akan mengambil transaksi milik pengguna itu sendiri.
        //    Jika pengguna adalah 'superadmin', kondisi where ini akan dilewati, sehingga ia bisa melihat semua transaksi.
        if (auth()->user()->role !== 'superadmin') {
            $query->where('user_id', $request->user()->user_id);
        }

        // 3. Menerapkan semua filter yang ada dari request
        ...
        // 4. Menghitung statistik berdasarkan query yang sudah difilter (SEBELUM pagination)
        //    Menggunakan clone() agar query utama untuk pagination tidak terpengaruh oleh sum().
        ...
        // 5. Mengurutkan hasil dan menerapkan pagination
        $transactions = $query->orderBy('created_at', 'desc')->paginate(15);
        
        // 6. Mengembalikan response dalam format JSON jika diminta
        ...
        // 7. Mengembalikan view dengan data transaksi dan statistik
