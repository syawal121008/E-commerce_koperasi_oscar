...
class Product extends Model
{
    ...
    public function scopeAvailableForPOS($query)
{
    return $query->where('stock', '>', 0)
                 ->where('is_active', true);
}

public function updateStockWithStatus($stockChange)
{
    $this->increment('stock', $stockChange);
    $this->syncStockStatus();
    return $this;
}

public static function getActivePOSProducts($adminId)
{
    ...
    // Return active products dengan stok
    ...
    {
        if ($this->stock > 0 && !$this->is_active) {
        $this->update(['is_active' => true]);
    } elseif ($this->stock <= 0 && $this->is_active) {
